...
2. Now update the "phishing.html" file your preferred phishing page source code.
...
3. Search for a preference named "security.csp.enable" and change its value to "false" by double clicking it to allow performing an XHR Request over a different domain (We're not supporting leaving this preference disabled, you may do that while testing, but after that, you should set the preference to its original state).
...
There is a lot of well-known web applications and Services which are vulnerable to this attack till the date we wrote this paper. Here're some examples (that we have reported) including but not limited to:
...
# Acknowledgements
I would like to personally thank the talented people who helped to shape the QRLJacking attack and to get it out to the light. (List in no particular order)
